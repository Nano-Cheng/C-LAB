...
def get_zi_func(x, MAX_VALUE=200):
...
def build_experiment(data, ch, step, target):
    ...
    for idx in data_ch.index.values:
        data.loc[idx, 'zi'] = get_zi_func(data.loc[idx, '2nd_peak_width'])
...
for target in [700]:
    ...
    for i in range(500):
        ...
        if step > final_step:
            final_step = step
            data[data['is_close']==1].sort_values(by='step', ascending=True).to_excel(f'./output/Astar_width_{target}.xlsx', header=True, index=False)
...
tdata = pd.read_excel('./output/Astar_width_700.xlsx', header=0)
tdata[['4mM AgNO3/mL',
       '3.6542M 盐酸/mL',
