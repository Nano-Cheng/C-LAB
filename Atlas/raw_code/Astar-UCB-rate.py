...
for target in [700]:
    ...
    for i in range(200):
        ...
        if step > final_step:
            final_step = step
            data[data['is_close']==1].sort_values(by='step', ascending=True).to_excel(f'./output/Astar_rate_{target}.xlsx', header=True, index=False)
...
tdata = pd.read_excel('./output/Astar_rate_750.xlsx', header=0)
tdata[['4mM AgNO3/mL',
       '3.6542M 盐酸/mL',
